...
@Composable
fun InspeccionLista(navController: NavController, inspeccion: Int, viewModel: ListModel) {
    ...
    BackHandler {
        coroutineScope.launch {
            navController.navigate("InspeccionMain")
        }
    }
            ...
            LazyColumn(
                ...
                itemsIndexed(detalles) { _, item ->
                    ListItem1(item) {
                        ...
                        navController.navigate("InspeccionSuministro/" + item.id.toString())
                    }
                }
            }
...
@Composable
fun ListPreview() {
    val previewViewModel = ListModel(DatabaseHelper(LocalContext.current))
    AppTheme {
